# -*- coding: utf-8 -*-
"""
Created on Tue Apr 25 15:55:06 2017
...
from metrics_mul import mul_score

def KFold_Mul_Experiment(estimator,kf,X_raw_train,y_raw_train,X_test, y_test ,Cali_method):
    cv_score_arr = []
    final_score_arr = []
    for train_index, cv_index in kf.split(X_raw_train):
        ...
        if Cali_method=='isotonic':
            ...
            cv_score = mul_score(estimator, X_cv, y_cv)
            final_score = mul_score(estimator, X_test, y_test)
...
def KNN_Classifier_Mul(X_raw_train,y_raw_train,X_test, y_test,Cali_method):
    k_range = range(1,30)       
    ...
    for k in k_range:
        for w in weight:
            for m in metrics:
                ...
                cv_score,final_score = KFold_Mul_Experiment(knn,kf, X_raw_train, y_raw_train,X_test, y_test,Cali_method)
                #cv
...
def LogisticRegression_Classifier_Mul(X_raw_train,y_raw_train,X_test, y_test,Cali_method):
    mean_cv_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    mean_final_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    ...
    for c in c_range:
        for sol in solvers:
            ...
            index_name.append(name)
            cv_score,final_score = KFold_Mul_Experiment(lr,kf, X_raw_train, y_raw_train,X_test, y_test,Cali_method)
            #cv
            cv_scores = pd.DataFrame(cv_score,columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
            cv_scores.loc['mean'] = cv_scores.apply(lambda x: x.mean())
            mean_cv_scores = cv_scores.loc['mean']
...
def Naive_Bayes_Classifier_Mul(X_raw_train,y_raw_train,X_test, y_test,Cali_method):
    mean_cv_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    mean_final_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    index_name = []
    ##采用5次五折交叉验证法
    ...
    name = 'GaussianNB'
    index_name.append(name)
    cv_score,final_score = KFold_Mul_Experiment(gnb,kf, X_raw_train, y_raw_train,X_test, y_test,Cali_method)
    #cv
    cv_scores = pd.DataFrame(cv_score,columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    cv_scores.loc['mean'] = cv_scores.apply(lambda x: x.mean())
    mean_cv_scores = cv_scores.loc['mean']
...
def DecisionTree_Classifier_Mul(X_raw_train,y_raw_train,X_test, y_test,Cali_method):
    mean_cv_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    mean_final_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    index_name = []
    ##采用5次五折交叉验证法
    ...
    for ct in criterions:
        ...
        name = 'criterion='+ct
        index_name.append(name)
        cv_score,final_score = KFold_Mul_Experiment(dt,kf, X_raw_train, y_raw_train,X_test, y_test,Cali_method)
        #cv
        cv_scores = pd.DataFrame(cv_score,columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
        cv_scores.loc['mean'] = cv_scores.apply(lambda x: x.mean())
        mean_cv_scores = cv_scores.loc['mean']
...
def RandomForest_Classifier_Mul(X_raw_train,y_raw_train,X_test, y_test,Cali_method):
    mean_cv_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    mean_final_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    index_name = []
    ##采用5次五折交叉验证法
    ...
    for ct in criterions:
        for feature in max_feature:
            ...
            index_name.append(name)
            cv_score,final_score = KFold_Mul_Experiment(rf,kf, X_raw_train, y_raw_train,X_test, y_test,Cali_method)
            #cv
            cv_scores = pd.DataFrame(cv_score,columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
            cv_scores.loc['mean'] = cv_scores.apply(lambda x: x.mean())
            mean_cv_scores = cv_scores.loc['mean']
...
def NeuralNetwork_Classifier_Mul(X_raw_train,y_raw_train,X_test, y_test,Cali_method):
    mean_cv_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    mean_final_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    index_name = []
    ...
    for h in hidden_layer:
        for a in activations:
            for s in solvers:
                ...
                index_name.append(name)
                cv_score,final_score = KFold_Mul_Experiment(mlp,kf, X_raw_train, y_raw_train,X_test, y_test,Cali_method)
                #cv
                cv_scores = pd.DataFrame(cv_score,columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
                cv_scores.loc['mean'] = cv_scores.apply(lambda x: x.mean())
                mean_cv_scores = cv_scores.loc['mean']
...
def SVM_Classifier_Mul(X_raw_train,y_raw_train,X_test, y_test,Cali_method):
    mean_cv_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    mean_final_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    index_name = []
    ##采用5次五折交叉验证法
    ...
    for c in c_range:
        for k in kernels:
            ...
            index_name.append(name)
            cv_score,final_score = KFold_Mul_Experiment(svc,kf, X_raw_train, y_raw_train,X_test, y_test,Cali_method)
            #cv
            cv_scores = pd.DataFrame(cv_score,columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
            cv_scores.loc['mean'] = cv_scores.apply(lambda x: x.mean())
            mean_cv_scores = cv_scores.loc['mean']
...
def Adaboost_Classifier_Mul(X_raw_train,y_raw_train,X_test, y_test,Cali_method):
    mean_cv_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    mean_final_scores_df = pd.DataFrame(columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
    index_name = []
    ##采用5次五折交叉验证法
    ...
    for b in base_estimators:
        for n in n_estimator:
            for r in learning_rates:
                ...
                index_name.append(name)
                cv_score,final_score = KFold_Mul_Experiment(b['estimator'],kf, X_raw_train, y_raw_train,X_test, y_test,Cali_method)
                #cv
                cv_scores = pd.DataFrame(cv_score,columns=['accuracy', 'average_precision', 'f1', 'roc_auc', 'RMSE','MXE','APaccuracy','BEP_score'])
                cv_scores.loc['mean'] = cv_scores.apply(lambda x: x.mean())
                mean_cv_scores = cv_scores.loc['mean']
