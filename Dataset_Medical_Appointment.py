# -*- coding: utf-8 -*-
"""
Created on Sat Apr 22 21:21:27 2017
...
data = pd.read_csv("E:\machine_learing\DateSet\Medical_Appointment.csv")

data.Gender = data.Gender.apply(lambda x: 1 if x == 'M' else 0)
data.Status = data.Status.apply(lambda x: 1 if x == 'Show-Up' else 0)
scaler_attr(data,'Age')
week = pd.get_dummies(data.DayOfTheWeek)
data = pd.merge(data,week,left_index=True,right_index=True,how='outer')
X = data[['Age_scaled', 'Diabetes','HiperTension', 'Tuberculosis', 'Smokes','Alcoolism', 'Scholarship','Friday','Monday','Saturday','Sunday','Thursday','Tuesday','Wednesday']]
y = data.Status
